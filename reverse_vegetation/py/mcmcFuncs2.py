...
# -- Function to call pySMARTS and produce a model
    ...
    albedo = albedoFunc(mywav, a2, b2, c2, a3, b3, c3, d)
...
def albedoFunc(wav, a2, b2, c2, a3, b3, c3, d):
...
#                 (b3*np.exp(-((wav-a3)**2)/(2*(c3**2)))) + d
        albedo = (b2*np.exp(-((wav-a2)**2)/(2*(c2**2)))) + \
                 (b3*np.exp(-((wav-a3)**2)/(2*(c3**2)))) + d
    except:
...
#    and multiply by the given amplitude
    ...
    try:
        ...
        #gauss_kernel = Gaussian1DKernel(1.0)
        #smooth_spec = convolve(model, gauss_kernel)
    except:
...
#    W, ApHNO2, ApNO2, ApNO3, AbO3, ApO3, ApSO2, TAU5, amp, eps = theta
    a2, b2, c2, a3, b3, c3, d, RH, W, ApHNO2, ApNO2, ApNO3, \
...
#    if (c1 <= 0.0) or (c1 > 1.0): 
#        return -np.inf
    if (a2 < 0.55) or (a2 >= 0.65):
        return -np.inf
    if (b2 <= 0) or (b2 > 0.1):
        return -np.inf
    if (c2 <= 0.02) or (c2 > 0.1): 
        return -np.inf
    if (a3 < 0.65) or (a3 >= 0.85):
        return -np.inf    
    if (b3 <= 0) or (b3 > 1.0):
        return -np.inf    
    if (c3 <= 0.0) or (c3 > 1.0): 
...
#    if (a4 < 0.7) or (a4 >= 0.8):
#        return -np.inf    
#    if (b4 <= 0.25) or (b4 > 0.75):
#        return -np.inf    
#    if (c4 <= 0.0) or (c4 > 0.5): 
#        return -np.inf
    nwav = np.linspace(0.38,0.83,91)
    albedo = albedoFunc(nwav, a2, b2, c2, a3, b3, c3, d)
...
#    TAU5 = 0.084
    ...
    modwav, modsmrt = modelFunc(scan, a2, b2, c2, a3, b3, c3, d, RH,
                                W, ApCH2O, ApCH4, ApCO, ApHNO2, ApHNO3, ApNO, ApNO2, ApNO3, AbO3, ApO3,
...
#    TAU5 = 0.084
    ...
    denom = eps**2
    lk = -0.5 * sum(((y[:-33] - model[:-33])**2) / denom + np.log(denom) + np.log(2*np.pi))
#    if lk > 0:
#        return -np.inf
    return lk
